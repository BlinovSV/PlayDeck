...
with st.sidebar:
    ...
    with st.expander('Единицы измерения и точность', expanded=False):
        ...
        with col_2:
            ...
            U_Forces = st.selectbox('Силы',['Н', 'кН', 'кгс', 'тс'], index=1, label_visibility='visible', disabled=False)
    ...
    UD_Forces = {
        ...
        'кгс': 0.101971621,
        'тс': 0.000101971621}
    UF_Forces = UD_Forces.get(U_Forces, 0.0) / UD_Forces.get('кН',0)
    ...
    with st.expander('Равнопролётная свободно опертая балка', expanded=True):
        ...
        load_uls = st.number_input('Нагрузка для проверок по ПС1', min_value=0.0 * UF_Load_Area, max_value=10000.0 * UF_Load_Area, value=1000.0 * UF_Load_Area, step=1.0 * UF_Load_Area) * UD_Load_Area.get('кПа',0.0) / UF_Load_Area
        load_sls = st.number_input('Нагрузка для проверок по ПС2', min_value=0.0 * UF_Load_Area, max_value=10000.0 * UF_Load_Area, value=1000.0 * UF_Load_Area, step=1.0 * UF_Load_Area) * UD_Load_Area.get('кПа',0.0) / UF_Load_Area
...
rho_s = 7850                                            # Плотность стали [кг/м³]
E_s = 206000                                            # Модуль упругости стали [МПа]
nu = 0.3                                                # Коэффициент поперечной деформации стали [-]
...
t = t_nom - 2 * t_coat                                  # Расчётная толщина стенки [м]
...
# Вычисление недостающих параметров геометрии профиля
...
alpha = m.asin((n_ws * h_ws) / s_w) + m.atan(h / b_w) # Угол наклона стенки к горизонтали [рад.]
beta = 0 if n_ws == 0 else alpha - m.pi / 2 + m.atan((h_s1 / m.tan(alpha) + h_ws / m.sin(alpha)) / h_s1) # Угол ребер стенки [рад.]
gamma_wfs = m.atan(2 * h_wfs / b_wfs) # Угол ребер широкой полки [рад.]
gamma_tfs = 0 if n_tfs == 0 else m.atan(2 * h_tfs / b_tfs) # Угол ребер узкой полки [рад.]
omega = 0 if n_ws == 0 else alpha - beta # Угол наклона ребер стенки к горизонтали [рад.]

s_1 = h_1 / m.sin(alpha) # Длина верхнего участка стенки [м]
s_2 = 0 if (n_ws == 0 or n_ws == 1) else h_2 / m.sin(alpha) # Длина среднего участка стенки [м]
s_3 = 0 if n_ws == 0 else h_3 / m.sin(alpha) # Длина нижнего участка стенки [м]
s_s1 = 0 if n_ws == 0 else (h_s1 / m.tan(alpha) + h_ws / m.sin(alpha)) / m.cos(omega) # Длина верхнего ребра [м]
...
elif (h / t_nom > 300) or (h / t_nom > 300 * m.sin(alpha)):
...
elif (0.25 > alpha / m.pi) or (alpha / m.pi > 0.5):
...
s_gr_n = h_gr_c / m.sin(alpha) if (n_ws == 0 or n_ws == 1) else (h_gr_c - h_1 - h_s1)/m.sin(alpha) if n_ws == 2 else None # Длина сжатой зоны участка стенки
...
delta = 0.43 * (4 * r * alpha/(m.pi/2) + n_ws * 4 * r_s * beta/(m.pi/2) + n_wfs * 4 * r_s * gamma_wfs/(m.pi/2) + n_tfs * 4 * r_s * gamma_tfs/(m.pi/2)) / L_gr # Коэффициент, учитывающий скругления профиля [-]
A_gr = L_gr * t * (1 - delta) # Площадь полного сечения на метр
P_gr = (L_gr + (b_tf + n_tfs * (2 * s_tfs - b_tfs))/n_cor) * t_nom * rho_s * 1 / b # Примерная масса квадратного метра профилированного настила

# Определение моментов инерции и моментов сопротивления полного сечения
if n_ws == 0 or n_ws == 1:
    I_gr = (((b_wf_pm * t**3)/12 + b_wf_pm * t * (h_gr_c)**2)
             + 2 * n_wfs * ((s_wfs * t**3)/12 * m.cos(gamma_wfs)**2 + (t * s_wfs**3)/12 * m.sin(gamma_wfs)**2 + s_wfs * t * (h_gr_c - h_wfs / 2)**2)
             + 2 * ((b_wf_ps * t**3)/12 + b_wf_ps * t * (h_gr_c)**2)
             + 2 * ((s_gr_n * t**3)/12 * m.cos(alpha)**2 + (t * s_gr_n**3)/12 * m.sin(alpha)**2 + s_gr_n * t * (s_gr_n * m.sin(alpha) / 2)**2)
             + 2 * ((s_gr_m * t**3)/12 * m.cos(alpha)**2 + (t * s_gr_m**3)/12 * m.sin(alpha)**2 + s_gr_m * t * (s_gr_m * m.sin(alpha) / 2)**2)
             + 2 * ((s_s1 * t**3)/12 * m.cos(omega)**2 + (t * s_s1**3)/12 * m.sin(omega)**2 + s_s1 * t * (h_gr_c - h_1 - h_s1 / 2)**2)
             + 2 * ((s_3 * t**3)/12 * m.cos(alpha)**2 + (t * s_3**3)/12 * m.sin(alpha)**2 + s_3 * t * (h - h_3 / 2 - h_gr_c)**2)
             + 2 * ((b_tf_ps * t**3)/12 + b_tf_ps * t * (h - h_gr_c)**2)
             + 2 * n_tfs * ((s_tfs * t**3)/12 * m.cos(gamma_tfs)**2 + (t * s_tfs**3)/12 * m.sin(gamma_tfs)**2 + s_tfs * t * (h - h_gr_c - h_tfs / 2)**2)) * 1 / b * (1 - 2 * delta) # Момент инерции полного сечения без ребер стенок с учётом скруглений
elif n_ws == 2:
    I_gr = (((b_wf_pm * t**3)/12 + b_wf_pm * t * (h_gr_c)**2)
            + 2 * n_wfs * ((s_wfs * t**3)/12 * m.cos(gamma_wfs)**2 + (t * s_wfs**3)/12 * m.sin(gamma_wfs)**2 + s_wfs * t * (h_gr_c - h_wfs / 2)**2)
            + 2 * ((b_wf_ps * t**3)/12 + b_wf_ps * t * (h_gr_c)**2)
            + 2 * ((s_1 * t**3)/12 * m.cos(alpha)**2 + (t * s_1**3)/12 * m.sin(alpha)**2 + s_1 * t * (h_gr_c - h_1 / 2)**2)
            + 2 * ((s_s1 * t**3)/12 * m.cos(omega)**2 + (t * s_s1**3)/12 * m.sin(omega)**2 + s_s1 * t * (h_gr_c - h_1 - h_s1 / 2)**2)
            + 2 * ((s_gr_n * t**3)/12 * m.cos(alpha)**2 + (t * s_gr_n**3)/12 * m.sin(alpha)**2 + s_gr_n * t * (s_gr_n * m.sin(alpha) / 2)**2)
            + 2 * ((s_gr_m * t**3)/12 * m.cos(alpha)**2 + (t * s_gr_m**3)/12 * m.sin(alpha)**2 + s_gr_m * t * (s_gr_m * m.sin(alpha) / 2)**2)
            + 2 * ((s_s2 * t**3)/12 * m.cos(omega)**2 + (t * s_s2**3)/12 * m.sin(omega)**2 + s_s2 * t * (h_1 + h_s1 + h_2 + h_s2 / 2 - h_gr_c)**2)
            + 2 * ((s_3 * t**3)/12 * m.cos(alpha)**2 + (t * s_3**3)/12 * m.sin(alpha)**2 + s_3 * t * (h - h_3 / 2 - h_gr_c)**2)
            + 2 * ((b_tf_ps * t**3)/12 + b_tf_ps * t * (h - h_gr_c)**2)
            + 2 * n_tfs * ((s_tfs * t**3)/12 * m.cos(gamma_tfs)**2 + (t * s_tfs**3)/12 * m.sin(gamma_tfs)**2 + s_tfs * t * (h - h_gr_c - h_tfs / 2)**2)) * 1 / b * (1 - 2 * delta) # Момент инерции полного сечения с двумя ребрами на стенке с учётом скруглений
...
def effective_section_properties(flange:str):
    ...
    if flange == 'Широкая':
        while abs(z - z_ini) > 0.000001:
            ...
            s_n = h_c / m.sin(alpha) if (n_ws == 0 or n_ws == 1) else (h_c - h_1 - h_s1)/m.sin(alpha) if n_ws == 2 else None # Длина сжатой зоны среднего участка стенки с учётом редукции
            ...
            sigma_ps_cr = k_sigma * (m.pi**2 * E_s * t**2)/(12 * (1 - nu**2) * b_wf_ps**2) # Критические напряжения потери устойчивости плоского участка широкой полки
            lambda_ps = m.sqrt(R_y/sigma_ps_cr) # Коэффициент для плоского участка широкой полки
            rho_ps = 1.0 if lambda_ps <= 0.673 else (lambda_ps - 0.055 * (3 + psi)) / (lambda_ps**2) # Коэффициент редукции для плоского участка широкой полки
            b_ps_ef = rho_ps * b_wf_ps # Эффективаня ширина плоского участка широкой полки
            ...
            sigma_pm_cr = k_sigma * (m.pi**2 * E_s * t**2)/(12 * (1 - nu**2) * b_wf_pm**2) if n_wfs == 2 else 0 # Критические напряжения потери устойчивости плоского участка широкой полки
            lambda_pm = m.sqrt(R_y/sigma_pm_cr) if n_wfs == 2 else 0 # Коэффициент для плоского участка широкой полки
            rho_pm = 1.0 if lambda_pm <= 0.673 else (lambda_pm - 0.055 * (3 + psi)) / (lambda_pm**2) if n_wfs == 2 else 0 # Коэффициент редукции для плоского участка широкой полки
            b_pm_ef = rho_pm * b_wf_pm if n_wfs == 2 else 0 # Эффективная ширина плоского участка широкой полки
            ...
            if n_wfs == 0:
                ...
                I_wfs = 2 * (((b_p_i + b_pm_i) * t**3) / 12 + ((b_p_i + b_pm_i) * t * h_tfs_c**2) + n_wfs * ((s_wfs * t**3)/12 * m.cos(gamma_wfs)**2 + (t * s_wfs**3)/12 * m.sin(gamma_wfs)**2 + (s_wfs * t * (h_wfs/2 - h_tfs_c)**2))) # Момент инерции элемента жесткости широкой полки
                
                # Критические напряжения потери устойчивости формы сечения полки:
                if n_wfs == 1:
                    sigma_cr_s = (4.2 * E_s)/A_s_i * m.sqrt((I_wfs * t**3)/(4 * b_wf_ps**2 * (2 * b_wf_ps + 3 * 2 * s_wfs)))
                elif n_wfs == 2:
                    b_l = b_wf_ps + b_wfs / 2
                    b_e = 2 * b_wf_ps + b_wf_pm + 4 * s_wfs
                    sigma_cr_s = (4.2 * E_s)/A_s_i * m.sqrt((I_wfs * t**3)/(8 * b_l**2 * (3 * b_e - 4 * b_l))) 
            ...
            if n_ws == 0 or n_ws == 1:
                sigma_cr_sa = 0 # Критические напряжения потери устойчивости формы сечения стенки
            else:
                if n_ws == 1:
                ...
                I_ws = (s_i3 * t**3)/12 + (s_i3 * t * h_ws_c**2) + (s_i2 * t**3)/12 + s_i2 * t * (h_ws - h_ws_c)**2 + (s_s1 * t**3)/12 * m.cos(beta)**2 + (t * s_s1**3)/12 * m.sin(beta)**2 + s_s1 * t * (h_ws/2 - h_ws_c)**2 # Момент инерции ребра стенки
                sigma_cr_sa = (1.05 * E_s * m.sqrt(I_ws * t**3 * s_l))/(A_sa_i * s_e * (s_l - s_e)) # Критические напряжения потери устойчивости формы сечения стенки

            # Учёт взаимного влияние ребер стенки и полки
            if n_ws == 2:
                beta_s = 1 - (h_1 + h_s1/2) / h_c # Коэффициент
                sigma_cr_mod = sigma_cr_s / (1 + (beta_s * sigma_cr_s/sigma_cr_sa)**4)**0.25 # Критические напряжения поетри устойчивости
            else:
                beta_s = 0 # Коэффициент
                sigma_cr_mod = sigma_cr_s # Критические напряжения потери устойчивости

            # Редукция толщин ребер полок и стенок
            lambda_d = m.sqrt(R_y/sigma_cr_mod) # Коэффициент
            if lambda_d <= 0.65: chi_d = 1.0
            elif 0.65 < lambda_d < 1.38: chi_d = 1.47 - 0.723 * lambda_d
            else: chi_d = 0.66/lambda_d

            t_ef_f = chi_d * t # Редуцированная толщина элемента жесткость полки
            if n_ws == 2:
                t_ef_w = min(chi_d * t * m.sqrt(h_c/(h_c - h_1 - h_s1/2)), t) # Редуцированная толщина элемента жесткости стенки
            else:
                t_ef_w = t

            # Геометрические характеристики сечения с учётом редукции
            h_ef_c = ((n_tfs * s_tfs * t * (h - h_tfs / 2) + b_tf_ps * t * h
                        + s_3 * t * (h - h_3 / 2) + s_s1 * t_ef_w * (h_1 + h_s1 / 2) + s_s2 * t * (h - h_3 - h_s2 / 2) + s_m * t * (h_c + s_m * m.sin(alpha) / 2) + s_ef_n * t * (h_c - s_ef_n * m.sin(alpha) / 2) + s_ef_3 * t_ef_w * (h_1 + h_s1 + s_ef_3 * m.sin(alpha) / 2) + s_ef_2 * t_ef_w * (h_1 - s_ef_2 * m.sin(alpha) / 2)
                        + s_ef_1 * t * (s_ef_1 * m.sin(alpha) / 2) + n_wfs * s_wfs * t_ef_f * (h_wfs / 2))
        ...
        if n_ws == 0:
            I_ef = (2 * n_tfs * ((s_tfs * t**3) / 12 * m.cos(gamma_tfs)**2 + (t * s_tfs**3) / 12 * m.sin(gamma_tfs)**2 + s_tfs * t * (h - h_ef_c - h_tfs / 2)**2)
                    + 2 * ((b_tf_ps * t**3) / 12 + b_tf_ps * t * (h - h_ef_c)**2)
                    + 2 * ((s_m * t**3) / 12 * m.cos(alpha)**2 + (t * s_m**3) / 12 * m.sin(alpha)**2 + s_m * t * (s_m * m.sin(alpha)/2)**2)
                    + 2 * ((s_ef_n * t**3) / 12 * m.cos(alpha)**2 + (t * s_ef_n**3) / 12 * m.sin(alpha)**2 + s_ef_n * t * (s_ef_n * m.sin(alpha)/2)**2)
                    + 2 * ((s_ef_1 * t**3) / 12 * m.cos(alpha)**2 + (t * s_ef_1**3) / 12 * m.sin(alpha)**2 + s_ef_1 * t * (h_ef_c - s_ef_1 * m.sin(alpha)/2)**2)
                    + 2 * ((b_ps_ef_e * t**3) / 12 + b_ps_ef_e * t * h_ef_c**2)
                    + 2 * ((b_ps_ef_e * t_ef_f**3) / 12 + b_ps_ef_e * t_ef_f * h_ef_c**2)
                    + 2 * n_wfs * ((s_wfs * t_ef_f**3) / 12 * m.cos(gamma_wfs)**2 + (t_ef_f * s_wfs**3) / 12 * m.sin(gamma_wfs)**2 + s_wfs * t_ef_f * (h_ef_c - h_wfs / 2)**2)
                    + 2 * ((b_pm_ef_e * t_ef_f**3) / 12 + b_pm_ef_e * t_ef_f * h_ef_c**2))* 1/b * (1 - 2 * delta) # Момент инерции эффективного сечения
        elif n_ws == 1:
            I_ef = (2 * n_tfs * ((s_tfs * t**3) / 12 * m.cos(gamma_tfs)**2 + (t * s_tfs**3) / 12 * m.sin(gamma_tfs)**2 + s_tfs * t * (h - h_ef_c - h_tfs / 2)**2)
                    + 2 * ((b_tf_ps * t**3) / 12 + b_tf_ps * t * (h - h_ef_c)**2)
                    + 2 * ((s_3 * t**3) / 12 * m.cos(alpha)**2 + (t * s_3**3)/ 12 * m.sin(alpha)**2 + s_3 * t * (h - h_ef_c - h_3 / 2)**2)
                    + 2 * ((s_s1 * t_ef_w**3) / 12 * m.cos(omega)**2 + (t_ef_w * s_s1**3) / 12 * m.sin(omega)**2 + s_s1 * t_ef_w * (h_ef_c - h_1 - h_s1 / 2)**2)
                    + 2 * ((s_m * t**3) / 12 * m.cos(alpha)**2 + (t * s_m**3) / 12 * m.sin(alpha)**2 + s_m * t * (s_m * m.sin(alpha)/2)**2)
                    + 2 * ((s_ef_n * t**3) / 12 * m.cos(alpha)**2 + (t * s_ef_n**3) / 12 * m.sin(alpha)**2 + s_ef_n * t * (s_ef_n * m.sin(alpha)/2)**2)
                    + 2 * ((s_ef_1 * t**3) / 12 * m.cos(alpha)**2 + (t * s_ef_1**3) / 12 * m.sin(alpha)**2 + s_ef_1 * t * (h_ef_c - s_ef_1 * m.sin(alpha)/2)**2)
                    + 2 * ((b_ps_ef_e * t**3) / 12 + b_ps_ef_e * t * h_ef_c**2)
            ...
            I_ef = (2 * n_tfs * ((s_tfs * t**3) / 12 * m.cos(gamma_tfs)**2 + (t * s_tfs**3) / 12 * m.sin(gamma_tfs)**2 + s_tfs * t * (h - h_ef_c - h_tfs / 2)**2)
                    + 2 * ((b_tf_ps * t**3) / 12 + b_tf_ps * t * (h - h_ef_c)**2)
                    + 2 * ((s_3 * t**3) / 12 * m.cos(alpha)**2 + (t * s_3**3)/ 12 * m.sin(alpha)**2 + s_3 * t * (h - h_ef_c - h_3 / 2)**2)
                    + 2 * ((s_s2 * t**3) / 12 * m.cos(omega)**2 + (t * s_s2**3) / 12 * m.sin(omega)**2 + s_s2 * t * (h - h_ef_c - h_3 - h_s2 / 2)**2)
                    + 2 * ((s_s1 * t_ef_w**3) / 12 * m.cos(omega)**2 + (t_ef_w * s_s1**3) / 12 * m.sin(omega)**2 + s_s1 * t_ef_w * (h_ef_c - h_1 - h_s1 / 2)**2)
                    + 2 * ((s_m * t**3) / 12 * m.cos(alpha)**2 + (t * s_m**3) / 12 * m.sin(alpha)**2 + s_m * t * (s_m * m.sin(alpha)/2)**2)
                    + 2 * ((s_ef_n * t**3) / 12 * m.cos(alpha)**2 + (t * s_ef_n**3) / 12 * m.sin(alpha)**2 + s_ef_n * t * (s_ef_n * m.sin(alpha)/2)**2)
                    + 2 * ((s_ef_3 * t_ef_w**3) / 12 * m.cos(alpha)**2 + (t_ef_w * s_ef_3**3) / 12 * m.sin(alpha)**2 + s_ef_3 * t_ef_w * (h_ef_c - h_1 - h_s1 - s_ef_3 * m.sin(alpha)/2)**2)
                    + 2 * ((s_ef_2 * t_ef_w**3) / 12 * m.cos(alpha)**2 + (t_ef_w * s_ef_2**3) / 12 * m.sin(alpha)**2 + s_ef_2 * t_ef_w * (h_ef_c - h_1 + s_ef_2 * m.sin(alpha)/2)**2)
                    + 2 * ((s_ef_1 * t**3) / 12 * m.cos(alpha)**2 + (t * s_ef_1**3) / 12 * m.sin(alpha)**2 + s_ef_1 * t * (h_ef_c - s_ef_1 * m.sin(alpha)/2)**2)
                    + 2 * ((b_ps_ef_e * t**3) / 12 + b_ps_ef_e * t * h_ef_c**2)
                    + 2 * ((b_ps_ef_e * t_ef_f**3) / 12 + b_ps_ef_e * t_ef_f * h_ef_c**2)
                    + 2 * n_wfs * ((s_wfs * t_ef_f**3) / 12 * m.cos(gamma_wfs)**2 + (t_ef_f * s_wfs**3) / 12 * m.sin(gamma_wfs)**2 + s_wfs * t_ef_f * (h_ef_c - h_wfs / 2)**2)
        ...
        while abs(z-z_ini) > 0.000001:
            ...
            s_n = h_c / m.sin(alpha) if n_ws == 0 else (h_c - h_3 - h_s1) / m.sin(alpha) if n_ws == 1 else (h_c - h_3 - h_s2) / m.sin(alpha) if n_ws == 2 else None # Длина сжатой зоны среднего участка стенки с учётом редукции
            s_m = s_1 - s_n if (n_ws == 0 or n_ws == 1) else (s_2 - s_n) if n_ws == 2 else None # Длина растянутой зоны среднего участка стенки с учётом редукции

            # Местная устойчивость сжатой узкой полки
            if n_tfs == 0:
                sigma_ps_cr = k_sigma * (m.pi**2 * E_s * t**2)/(12 * (1 - nu**2) * (2 * b_tf_ps)**2) # Критические напряжения потери устойчивости плоского участка узкой полки
                lambda_ps = m.sqrt(R_y / sigma_ps_cr) # Коэффициент для плоского участка узкой полки
                rho_ps = 1.0 if lambda_ps <= 0.673 else (lambda_ps - 0.055 * (3 + psi)) / (lambda_ps**2) # Коэффициент редукции для плоского участка узкой полки
                b_ps_ef = rho_ps * (2 * b_tf_ps) # Эффективаня ширина плоского участка узкой полки
                b_ps_ef_e = b_ps_ef / 2 # Половина ширины плоского участка широкой полки
            elif n_tfs == 1:
                sigma_ps_cr = k_sigma * (m.pi**2 * E_s * t**2)/(12 * (1 - nu**2) * b_tf_ps**2) # Критические напряжения потери устойчивости плоского участка узкой полки
                lambda_ps = m.sqrt(R_y / sigma_ps_cr) # Коэффициент для плоского участка узкой полки
                rho_ps = 1.0 if lambda_ps <= 0.673 else (lambda_ps - 0.055 * (3 + psi)) / (lambda_ps**2) # Коэффициент редукции для плоского участка узкой полки
                b_ps_ef = rho_ps * b_tf_ps # Эффективаня ширина плоского участка узкой полки
                b_ps_ef_e = b_ps_ef / 2 # Половина ширины плоского участка широкой полки   
            b_pm_ef_e = 0.0         
            ...
            if n_tfs == 0:
                ...
                I_tfs = 2 * ((b_p_i * t**3) / 12 + (b_p_i * t * h_tfs_c**2) + (s_tfs * t**3)/12 * m.cos(gamma_tfs)**2 + (t * s_tfs**3)/12 * m.sin(gamma_tfs)**2 + (s_tfs * t * (h_tfs/2 - h_tfs_c)**2)) # Момент инерции элемента жесткости широкой полки
                sigma_cr_s = (4.2 * E_s)/A_s_i * m.sqrt((I_tfs * t**3) / (4 * b_tf_ps**2 * (2 * b_tf_ps + 3 * 2 * s_tfs))) # Критические напряжения потери устойчивости формы сечения полки
            ...
            if n_ws == 0:
                ...
                if n_ws == 1:
                    ...
                    I_ws = (s_i3 * t**3)/12 + (s_i3 * t * h_ws_c**2) + (s_i2 * t**3)/12 + s_i2 * t * (h_ws - h_ws_c)**2 + (s_s2 * t**3)/12 * m.cos(beta)**2 + (t * s_s2**3)/12 * m.sin(beta)**2 + s_s2 * t * (h_ws/2 - h_ws_c)**2 # Момент инерции ребра стенки
                sigma_cr_sa = (1.05 * E_s * m.sqrt(I_ws * t**3 * s_l))/(A_sa_i * s_e * (s_l - s_e)) # Критические напряжения потери устойчивости формы сечения стенки

            # Учёт взаимного влияние ребер стенки и полки
            if n_tfs == 0:
                beta_s = 0
                sigma_cr_mod = sigma_cr_sa
            elif n_tfs == 1:
                if n_ws == 0:
                    sigma_cr_mod = sigma_ps_cr
                elif n_ws == 1:
                    beta_s = 1 - (h_3 + h_s1 / 2) / h_c # Коэффициент
                    sigma_cr_mod = sigma_cr_s / (1 + (beta_s * sigma_cr_s/sigma_cr_sa)**4)**0.25 # Критические напряжения поетри устойчивости
                elif n_ws == 2:
                    beta_s = 1 - (h_3 + h_s2 / 2) / h_c # Коэффициент
                    sigma_cr_mod = sigma_cr_s / (1 + (beta_s * sigma_cr_s/sigma_cr_sa)**4)**0.25 # Критические напряжения поетри устойчивости
            ...
            if n_tfs == 0:
                ...
                t_ef_f = chi_d * t # Редуцированная толщина полки
            else: t_ef_f = 0
            
            if n_ws == 0:
                t_ef_w = 0
            elif n_ws == 1:
                t_ef_w = min(chi_d * t * m.sqrt(h_c/(h_c - h_3 - h_s1/2)), t) # Редуцированная толщина элемента жесткости стенки
            elif n_ws == 2:
                t_ef_w = min(chi_d * t * m.sqrt(h_c/(h_c - h_3 - h_s2/2)), t) # Редуцированная толщина элемента жесткости стенки

            # Геометрические характеристики сечения с учётом редукции
            if n_ws == 0:
                h_ef_c = ((b_wf_pm / 2 * t * h + n_wfs * s_wfs * t * (h - h_wfs / 2) + b_wf_ps * t * h + s_m * t * (h_c + s_m * m.sin(alpha)/2) + s_ef_n * t * (h_c - s_ef_n * m.sin(alpha)/2) + s_ef_1 * t * (s_ef_1 * m.sin(alpha)/2) + n_tfs * s_tfs * t_ef_f * (h_tfs / 2))
                ...
                h_ef_c = ((b_wf_pm / 2 * t * h + n_wfs * s_wfs * t * (h - h_wfs / 2) + b_wf_ps * t * h
                            + s_m * t * (h_c + s_m * m.sin(alpha)/2) + s_ef_n * t * (h_c - s_ef_n * m.sin(alpha)/2) + s_ef_3 * t_ef_w * (h_3 + h_s1 + s_ef_3 * m.sin(alpha)/2) + s_s1 * t_ef_w * (h_3 + h_s1 / 2) + s_ef_2 * t_ef_w * (h_3 - s_ef_2 * m.sin(alpha)/2) + s_ef_1 * t * (s_ef_1 * m.sin(alpha)/2)
                ...
                h_ef_c = ((b_wf_pm / 2 * t * h + n_wfs * s_wfs * t * (h - h_wfs / 2) + b_wf_ps * t * h
                            + s_1 * t * (h - h_1 / 2) + s_s1 * t * (h - h_1 - h_s1 / 2) + s_m * t * (h_c + s_m * m.sin(alpha)/2) + s_ef_n * t * (h_c - s_ef_n * m.sin(alpha)/2)
                            + s_ef_3 * t_ef_w * (h_3 + h_s2 + s_ef_3 * m.sin(alpha)/2) + s_s2 * t_ef_w * (h_3 + h_s2 / 2) + s_ef_2 * t_ef_w * (h_3 - s_ef_2 * m.sin(alpha)/2)
                            + s_ef_1 * t * (s_ef_1 * m.sin(alpha)/2) + n_tfs * s_tfs * t_ef_f * (h_tfs / 2))
        ...
        if n_ws == 0:
            I_ef = (((b_wf_pm * t**3) / 12 + b_wf_pm * t * (h - h_ef_c)**2)
                + 2 * n_wfs * ((s_wfs * t**3) / 12 * m.cos(gamma_wfs)**2 + (t * s_wfs**3) / 12 * m.sin(gamma_wfs)**2 + s_wfs * t * (h - h_ef_c - h_wfs / 2)**2)
                ...
                + 2 * ((b_ps_ef_e * t**3) / 12 + b_ps_ef_e * t * (h_ef_c)**2)
                + 2 * ((b_ps_ef_e * t_ef_f**3) / 12 + b_ps_ef_e * t_ef_f * (h_ef_c)**2)
                + 2 * n_tfs * ((s_tfs * t_ef_f**3) / 12 * m.cos(gamma_tfs)**2 + (t_ef_f * s_tfs**3) / 12 * m.sin(gamma_tfs)**2 + s_tfs * t_ef_f * (h_ef_c - h_tfs / 2)**2)) * 1/b * (1 - 2 * delta) # Момент инерции редуцированного сечения
        elif n_ws == 1:
            I_ef = (((b_wf_pm * t**3) / 12 + b_wf_pm * t * (h - h_ef_c)**2)
                + 2 * n_wfs * ((s_wfs * t**3) / 12 * m.cos(gamma_wfs)**2 + (t * s_wfs**3) / 12 * m.sin(gamma_wfs)**2 + s_wfs * t * (h - h_ef_c - h_wfs / 2)**2)
                + 2 * ((b_wf_ps * t**3) / 12 + b_wf_ps * t * (h - h_ef_c)**2)
                + 2 * ((s_m * t**3) / 12 * m.cos(alpha)**2 + (t * s_m**3) / 12 * m.sin(alpha)**2 + s_m * t * (s_m * m.sin(alpha)/2)**2)
                + 2 * ((s_ef_n * t**3) / 12 * m.cos(alpha)**2 + (t * s_ef_n**3) / 12 * m.sin(alpha)**2 + s_ef_n * t * (s_ef_n * m.sin(alpha)/2)**2)
                + 2 * ((s_ef_3 * t_ef_w**3) / 12 * m.cos(alpha)**2 + (t_ef_w * s_ef_3**3) / 12 * m.sin(alpha)**2 + s_ef_3 * t_ef_w * (h_ef_c - h_3 - h_s1 - s_ef_3 * m.sin(alpha)/2)**2)
                + 2 * ((s_s1 * t_ef_w**3) / 12 * m.cos(omega)**2 + (t_ef_w * s_s1**3) / 12 * m.sin(omega)**2 + s_s1 * t_ef_w * (h_ef_c - h_3 - h_s1 / 2)**2)
                + 2 * ((s_ef_2 * t_ef_w**3) / 12 * m.cos(alpha)**2 + (t_ef_w * s_ef_2**3) / 12 * m.sin(alpha)**2 + s_ef_2 * t_ef_w * (h_ef_c - h_3 + s_ef_2 * m.sin(alpha)/2)**2)
                + 2 * ((s_ef_1 * t**3) / 12 * m.cos(alpha)**2 + (t * s_ef_1**3) / 12 * m.sin(alpha)**2 + s_ef_1 * t * (h_ef_c - s_ef_1 * m.sin(alpha)/2)**2)
                + 2 * ((b_ps_ef_e * t**3) / 12 + b_ps_ef_e * t * (h_ef_c)**2)
                + 2 * ((b_ps_ef_e * t_ef_f**3) / 12 + b_ps_ef_e * t_ef_f * (h_ef_c)**2)
                + 2 * n_tfs * ((s_tfs * t_ef_f**3) / 12 * m.cos(gamma_tfs)**2 + (t_ef_f * s_tfs**3) / 12 * m.sin(gamma_tfs)**2 + s_tfs * t_ef_f * (h_ef_c - h_tfs / 2)**2)) * 1/b * (1 - 2 * delta) # Момент инерции редуцированного сечения
            ...
            I_ef = (((b_wf_pm * t**3) / 12 + b_wf_pm * t * (h - h_ef_c)**2)
                + 2 * n_wfs * ((s_wfs * t**3) / 12 * m.cos(gamma_wfs)**2 + (t * s_wfs**3) / 12 * m.sin(gamma_wfs)**2 + s_wfs * t * (h - h_ef_c - h_wfs / 2)**2)
                + 2 * ((b_wf_ps * t**3) / 12 + b_wf_ps * t * (h - h_ef_c)**2)
                + 2 * ((s_1 * t**3) / 12 * m.cos(alpha)**2 + (t * s_1**3) / 12 * m.sin(alpha)**2 + s_1 * t * (h - h_ef_c - h_1 / 2)**2)
                + 2 * ((s_s1 * t**3) / 12 * m.cos(omega)**2 + (t * s_s1**3) / 12 * m.sin(omega)**2 + s_s1 * t * (h - h_ef_c - h_1 - h_s1 / 2)**2)
                + 2 * ((s_m * t**3) / 12 * m.cos(alpha)**2 + (t * s_m**3) / 12 * m.sin(alpha)**2 + s_m * t * (s_m * m.sin(alpha)/2)**2)
                + 2 * ((s_ef_n * t**3) / 12 * m.cos(alpha)**2 + (t * s_ef_n**3) / 12 * m.sin(alpha)**2 + s_ef_n * t * (s_ef_n * m.sin(alpha)/2)**2)
                + 2 * ((s_ef_3 * t_ef_w**3) / 12 * m.cos(alpha)**2 + (t_ef_w * s_ef_3**3) / 12 * m.sin(alpha)**2 + s_ef_3 * t_ef_w * (h_ef_c - h_3 - h_s2 - s_ef_3 * m.sin(alpha)/2)**2)
                + 2 * ((s_s2 * t_ef_w**3) / 12 * m.cos(omega)**2 + (t_ef_w * s_s2**3) / 12 * m.sin(omega)**2 + s_s2 * t_ef_w * (h_ef_c - h_3 - h_s2 / 2)**2)
                + 2 * ((s_ef_2 * t_ef_w**3) / 12 * m.cos(alpha)**2 + (t_ef_w * s_ef_2**3) / 12 * m.sin(alpha)**2 + s_ef_2 * t_ef_w * (h_ef_c - h_3 + s_ef_2 * m.sin(alpha)/2)**2)
                + 2 * ((s_ef_1 * t**3) / 12 * m.cos(alpha)**2 + (t * s_ef_1**3) / 12 * m.sin(alpha)**2 + s_ef_1 * t * (h_ef_c - s_ef_1 * m.sin(alpha)/2)**2)
                + 2 * ((b_ps_ef_e * t**3) / 12 + b_ps_ef_e * t * (h_ef_c)**2)
                + 2 * ((b_ps_ef_e * t_ef_f**3) / 12 + b_ps_ef_e * t_ef_f * (h_ef_c)**2)
...
def capacity_moment(flange:str):
...
def capacity_shear():
    if orient == 'Вверх':
        ...
        if n_ws == 0:
            ...
            y_w = (h * s_3 * (h_s1 * m.cos(alpha) - s_s1 * m.sin(alpha) * m.cos(omega)))/(h_s1 * b_w - h * s_s1 * m.cos(omega)) # Вертикальная координата пересечения прямой с ребром стенки
            ...
            y_w = (h * s_3 * (h_s2 * m.cos(alpha) - s_s2 * m.sin(alpha) * m.cos(omega)))/(h_s2 * b_w - h * s_s2 * m.cos(omega)) # Вертикальная координата пересечения прямой с ребром стенки
            x_w = (y_w * b_w)/h # Горизонтальная координата пересечения прямой с ребром стенки
            e_min = (n_ws * h_ws * m.sqrt(x_w**2 + y_w**2))/m.sqrt(b_w**2 + h**2)
            e_max = h_ws - e_min
            k_a_s = min(1.45 - 0.05 * e_max/t, 0.95 + 35000 * t**2 * e_min/(s_3*b_d**2)) # Коэффициент, учитывающий наличие ребер на стенки
    ...
    # Несущая способность по поперечной силе
        if Q_en:
            Q_w_p = 0.15 * t**2 * m.sqrt(R_y * E_s * 10**6) * (1 - 0.1 * m.sqrt(r_s/t)) * (0.5 + m.sqrt(0.02*l_a/t)) * (2.4 + (alpha/(m.pi/2))**2) * k_a_s # Критическая поперечная сила потери устойчивости одной стенки гофра по EN 1993-1-3
        else: Q_w_p = C * t**2 * R_y * m.sin(alpha) * (1 - C_r * m.sqrt(r/t)) * (1 + C_b * m.sqrt(l_a/t)) * (1 - C_h * m.sqrt(h_1/t)) * k_a_s * 1e3 # Критическая поперечная сила потери устойчивости одной стенки гофра по СП 260.1325800.2023
        ...
        if n_ws == 0:
            ...
            y_w = (h * s_1 * (h_s1 * m.cos(alpha) - s_s1 * m.sin(alpha) * m.cos(omega)))/(h_s1 * b_w - h * s_s1 * m.cos(omega)) # Вертикальная координата пересечения прямой с ребром стенки
            x_w = (y_w * b_w)/h # Горизонтальная координата пересечения прямой с ребром стенки
        ...
        # Несущая способность по поперечной силе
        if Q_en:
            Q_w_p = 0.15 * t**2 * m.sqrt(R_y * E_s * 10**6) * (1 - 0.1 * m.sqrt(r_s/t)) * (0.5 + m.sqrt(0.02*l_a/t)) * (2.4 + (alpha/(m.pi/2))**2) * k_a_s # Критическая поперечная сила потери устойчивости одной стенки гофра
        else: Q_w_p = C * t**2 * R_y * m.sin(alpha) * (1 - C_r * m.sqrt(r/t)) * (1 + C_b * m.sqrt(l_a/t)) * (1 - C_h * m.sqrt(h_3/t)) * k_a_s * 1e3 
...
### ГРАФИК СЕЧЕНИЯ ###
def draw_section(flange:str, result:list):
    fig = go.Figure()
        ...
        if flange == 'Широкая':
            if n_wfs == 1 and n_ws == 0 and n_tfs == 0:
                main_points = [
                    [(-b / 2 , 0),
                    (-b / 2 + b_tf / 2, 0),
                    (-b / 2 + b_tf / 2 + (s_m + s_ef_n) * m.cos(alpha),(s_m + s_ef_n) * m.sin(alpha))],
                    [(-b_wf / 2 - s_ef_1 * m.cos(alpha), h - s_ef_1 * m.sin(alpha)),
                    (-b_wf / 2, h),
                    (-b_wf / 2 + b_ps_ef_e, h)]]
                reduced_segments = [
                    [(-b_ps_ef_e - b_wfs / 2, h),
                ...
                main_points = [
                    [(-b / 2 , 0),
                    (-b / 2 + b_tf / 2, 0),
                    (-b / 2 + b_tf / 2 + s_3 * m.cos(alpha), s_3 * m.sin(alpha)),
                    (-b / 2 + b_tf / 2 + s_3 * m.cos(alpha) + s_s1 * m.cos(omega), s_3 * m.sin(alpha) + s_s1 * m.sin(omega)),
                    (-b / 2 + b_tf / 2 + (s_3 + s_m + s_ef_n) * m.cos(alpha) + s_s1 * m.cos(omega), (s_3 + s_m + s_ef_n) * m.sin(alpha) + s_s1 * m.sin(omega))],
                    [(-b_wf / 2 - s_ef_1 * m.cos(alpha), h - s_ef_1 * m.sin(alpha)),
                    (-b_wf / 2, h),
                    (-b_wf / 2 + b_ps_ef_e, h)]]
                reduced_segments = [
                ...
                main_points = [
                    ...
                    (-b / 2 + b_tfs / 2, 0),
                    (-b / 2 + b_tf / 2, 0),
                    (-b / 2 + b_tf / 2 + s_3 * m.cos(alpha), h_3),
                    (-b / 2 + b_tf / 2 + s_3 * m.cos(alpha) + s_s2 * m.cos(omega), h_3 + h_s2),
                    (-b / 2 + b_tf / 2 + (s_3 + s_m + s_ef_n) * m.cos(alpha) + s_s2 * m.cos(omega), h_3 + h_s2 + (s_m + s_ef_n) * m.sin(alpha))],
                    [(-b_wf / 2 - s_ef_1 * m.cos(alpha), h - s_ef_1 * m.sin(alpha)),
                    (-b_wf / 2, h),
                    (-b_wf / 2 + b_ps_ef_e, h)]]
                reduced_segments = [
                    [(-b_wf / 2 - (s_1 + s_ef_3) * m.cos(alpha) - s_s1 * m.cos(omega), h - h_1 - h_s1 - s_ef_3 * m.sin(alpha)),
                    (-b_wf / 2 - s_1 * m.cos(alpha) - s_s1 * m.cos(omega), h - h_1 - h_s1),
                    (-b_wf / 2 - s_1 * m.cos(alpha), h - h_1),
                    (-b_wf / 2 - s_1 * m.cos(alpha) + s_ef_2 * m.cos(alpha), h - h_1 + s_ef_2 * m.sin(alpha))],
                    [(-b_wfs_s / 2 - b_wfs / 2 - b_ps_ef_e, h),
                    (-b_wfs_s / 2 - b_wfs / 2, h),
                    (-b_wfs_s / 2, h - h_wfs),
                    (-b_wfs_s / 2 + b_wfs / 2, h),
                ...
                main_points = [
                    ...
                    (-b/2 + b_tf/2 + s_3 * m.cos(alpha) + s_s2 * m.cos(omega), h_3 + h_s2),
                    (-b/2 + b_tf/2 + (s_3 + s_m + s_ef_n) * m.cos(alpha) + s_s2 * m.cos(omega), h_3 + h_s2 + (s_m + s_ef_n) * m.sin(alpha))],
                    [(-b_wfs/2 - b_wf_ps - s_ef_1 * m.cos(alpha), h - s_ef_1 * m.sin(alpha)),
                    (-b_wfs/2 - b_wf_ps, h), 
                    (-b_wfs/2 - b_wf_ps + b_ps_ef_e, h)]]
                reduced_segments = [
                    [(-b_wfs/2 - b_wf_ps - (s_1 + s_ef_3) * m.cos(alpha) - s_s1 * m.cos(omega), h - h_1 - h_s1 - s_ef_3 * m.sin(alpha)),
                    (-b_wfs/2 - b_wf_ps - s_1 * m.cos(alpha) - s_s1 * m.cos(omega), h - h_1 - h_s1),
                    (-b_wfs/2 - b_wf_ps - s_1 * m.cos(alpha), h - h_1),
                    (-b_wfs/2 - b_wf_ps - s_1 * m.cos(alpha) + s_ef_2 * m.cos(alpha), h - h_1 + s_ef_2 * m.sin(alpha))],
                    [(-b_wfs/2 - b_ps_ef_e, h),
                    (-b_wfs/2, h),
                    (0, h - h_wfs)]]
                inactive_segments = [
            ...
            if n_wfs == 1 and n_ws == 0 and n_tfs == 0:
                main_points = [
                    ...
                    (-b / 2 + b_wfs / 2, h),
                    (-b / 2 + b_wf / 2, h),
                    (-b / 2 + b_wf / 2 + (s_m + s_ef_n) * m.cos(alpha), h - (s_m + s_ef_n) * m.sin(alpha))],
                    [(-b_tf / 2 - s_ef_1 * m.cos(alpha), s_ef_1 * m.sin(alpha)),
                    (-b_tf / 2, 0),
                    (-b_tf / 2 + b_ps_ef_e,0)]]
                reduced_segments = []
                inactive_segments = [
                ...
                main_points = [
                    ...
                    (-b_tf / 2 + b_ps_ef_e, 0)]]
                reduced_segments = [
                    [(-b_tf / 2 - (s_3 + s_ef_3) * m.cos(alpha) - s_s1 * m.cos(omega), h_3 + h_s1 + s_ef_3 * m.sin(alpha)),
                    (-b_tf / 2 - s_3 * m.cos(alpha) - s_s1 * m.cos(omega), h_3 + h_s1),
                    (-b_tf / 2 - s_3 * m.cos(alpha), h_3),
                    (-b_tf / 2 - s_3 * m.cos(alpha) + s_ef_2 * m.cos(alpha), h_3 - s_ef_2 * m.sin(alpha))]]
                inactive_segments = [
                    [(-b/2 + b_wf / 2 + (s_m + s_ef_n) * m.cos(alpha), h - (s_m + s_ef_n) * m.sin(alpha)),
                    (-b_tf / 2 - (s_3 + s_ef_3) * m.cos(alpha) - s_s1 * m.cos(omega), h_3 + h_s1 + s_ef_3 * m.sin(alpha))],
                    [(-b_tf / 2 - s_3 * m.cos(alpha) + s_ef_2 * m.cos(alpha), h_3 - s_ef_2 * m.sin(alpha)),
                ...
                main_points = [
                    ...
                    (-b / 2 + b_wf_pm / 2 + b_wfs, h),
                    (-b / 2 + b_wf / 2, h),
                    (-b / 2 + b_wf / 2 + s_1 * m.cos(alpha), h - s_1 * m.sin(alpha)),
                    (-b / 2 + b_wf / 2 + s_1 * m.cos(alpha) + s_s2 * m.cos(omega), h - s_1 * m.sin(alpha) - h_s2),
                    (-b / 2 + b_wf / 2 + (s_1 + s_m + s_ef_n) * m.cos(alpha) + s_s2 * m.cos(omega), h - (s_1 + s_m + s_ef_n) * m.sin(alpha) - h_s2)],
                    [(-b_tf / 2 - s_ef_1 * m.cos(alpha), s_ef_1 * m.sin(alpha)),
                    (-b_tf / 2, 0),
                    (-b_tf / 2 + b_ps_ef_e, 0)]]
                reduced_segments = [
                ...
                main_points = [
                    ...
                    (-b/2 + b_wfs/2 + b_wf_ps + s_1 * m.cos(alpha), h - h_1),
                    (-b/2 + b_wfs/2 + b_wf_ps + s_1 * m.cos(alpha) + s_s1 * m.cos(omega), h - h_1 - h_s1),
                    (-b/2 + b_wfs/2 + b_wf_ps + (s_1 + s_m + s_ef_n) * m.cos(alpha) + s_s1 * m.cos(omega), h - h_1 - h_s1 - (s_m + s_ef_n) * m.sin(alpha))],
                    [(-b_tf/2 - s_ef_1 * m.cos(alpha), s_ef_1 * m.sin(alpha)),
                    (-b_tf/2, 0),
                    (-b_tf/2 + b_ps_ef_e, 0)]]
                reduced_segments = [
                    [(-b_tf/2 - (s_3 + s_ef_3) * m.cos(alpha) - s_s2 * m.cos(omega), h_3 + h_s2 + s_ef_3 * m.sin(alpha)),
                    (-b_tf/2 - s_3 * m.cos(alpha) - s_s2 * m.cos(omega), h_3 + h_s2),
                    (-b_tf/2 - s_3 * m.cos(alpha), h_3),
                    (-b_tf/2 - s_3 * m.cos(alpha) + s_ef_2 * m.cos(alpha), h_3 - s_ef_2 * m.sin(alpha))]]
                inactive_segments = [
                    [(-b / 2 + b_wf / 2 + (s_1 + s_m + s_ef_n) * m.cos(alpha) + s_s2 * m.cos(omega), h - (s_1 + s_m + s_ef_n) * m.sin(alpha) - h_s2),
        ...
        if flange == 'Широкая':
            if n_wfs == 1 and n_ws == 0 and n_tfs == 0:
                main_points = [
                    [(-b / 2, h),
                     (-b / 2 + b_tf / 2, h),
                     (-b / 2 + b_tf / 2 + (s_m + s_ef_n) * m.cos(alpha), h - (s_m + s_ef_n) * m.sin(alpha))],
                    [(-b_wf / 2 - s_ef_1 * m.cos(alpha), s_ef_1 * m.sin(alpha)),
                     (-b_wf / 2, 0),
                     (-b_wf / 2 + b_ps_ef_e, 0)]]
                reduced_segments = [
                    [(-b_wfs / 2  - b_ps_ef_e, 0),
                ...
                main_points = [
                    [(-b/2, h),
                    (-b / 2 + b_tf / 2, h),
                    (-b / 2 + b_tf / 2 + s_3 * m.cos(alpha), h - h_3),
                    (-b / 2 + b_tf / 2 + s_3 * m.cos(alpha) + s_s1 * m.cos(omega), h - h_3 - h_s1),
                    (-b/2 + b_tf / 2 + (s_3 + s_m + s_ef_n) * m.cos(alpha) + s_s1 * m.cos(omega), h - h_3 - h_s1 - (s_m + s_ef_n) * m.sin(alpha))],
                    [(-b_wf / 2 - s_ef_1 * m.cos(alpha), s_ef_1 * m.sin(alpha)),
                    (-b_wf / 2, 0),
                    (-b_wf / 2 + b_ps_ef_e, 0)]]
                reduced_segments = [
                ...
                main_points = [
                    [(-b / 2, h - h_tfs),
                     (-b / 2 + b_tfs / 2, h),
                     (-b / 2 + b_tf / 2, h),
                     (-b / 2 + b_tf / 2 + h_3 * m.cos(alpha), h - h_3),
                     (-b / 2 + b_tf / 2 + s_3 * m.cos(alpha) + s_s1 * m.cos(omega), h - h_3 - h_s1),
                     (-b / 2 + b_tf / 2 + (s_3 + s_m + s_ef_n) * m.cos(alpha) + s_s1 * m.cos(omega), h - h_3 - h_s1 - (s_m + s_ef_n) * m.sin(alpha))],
                    [(-b_wf / 2 - s_ef_1 * m.cos(alpha), s_ef_1 * m.sin(alpha)),
                     (-b_wf / 2, 0),
                ...
                main_points = [
                    [(-b / 2, h - h_tfs),
                     ...
                     (-b / 2 + b_tf / 2, h),
                     (-b / 2 + b_tf / 2 + s_3 * m.cos(alpha), h - h_3),
                     (-b / 2 + b_tf / 2 + s_3 * m.cos(alpha) + s_s2 * m.cos(omega), h - h_3 - h_s2),
                     (-b / 2 + b_tf / 2 + (s_3 + s_m + s_ef_n) * m.cos(alpha) + s_s2 * m.cos(omega), h - h_3 - h_s2 - (s_m + s_ef_n) * m.sin(alpha))],
                    [(-b_wf / 2 - s_ef_1 * m.cos(alpha), s_ef_1 * m.sin(alpha)),
                     (-b_wf / 2, 0),
                     (-b_wf / 2 + b_ps_ef_e, 0)]]
                reduced_segments = [
                    [(-b_wf / 2 - (s_1 + s_ef_3) * m.cos(alpha) - s_s1 * m.cos(omega), h_1 + h_s1 + s_ef_3 * m.sin(alpha)),
                     (-b_wf / 2 - s_1 * m.cos(alpha) - s_s1 * m.cos(omega), h_1 + h_s1),
                     (-b_wf / 2 - s_1 * m.cos(alpha), h_1),
                     (-b_wf / 2 - s_1 * m.cos(alpha) + s_ef_2 * m.cos(alpha), h_3 - s_ef_2 * m.sin(alpha))],
                    [(-b_wfs_s / 2 - b_wfs / 2 - b_ps_ef_e, 0),
                     (-b_wfs_s / 2 - b_wfs / 2, 0),
                     (-b_wfs_s / 2, h_tfs),
                     (-b_wf_pm / 2, 0),
                ...
                reduced_segments = [
                    [(-b_wf / 2 - (s_1 + s_ef_3) * m.cos(alpha) - s_s1 * m.cos(omega), h_1 + h_s1 + s_ef_3 * m.sin(alpha)),
                     (-b_wf / 2 - s_1 * m.cos(alpha) - s_s2 * m.cos(omega), h_1 + h_s1),
                     (-b_wf / 2 - s_1 * m.cos(alpha), h_3),
                     (-b_wf / 2 - s_1 * m.cos(alpha) + s_ef_2 * m.cos(alpha), h_3 - s_ef_2 * m.sin(alpha))],
                    [(-b_wfs / 2 - b_ps_ef_e, 0),
                     (-b_wfs / 2, 0),
                     (0, h_wfs)]]
                ...
                main_points = [
                    [(-b / 2, h),
                     (-b / 2 + b_tf / 2, h),
                     (-b / 2 + b_tf / 2 + s_3 * m.cos(alpha), h - s_3 * m.sin(alpha)),
                     (-b / 2 + b_tf / 2 + s_3 * m.cos(alpha) + s_s2 * m.cos(omega), h - s_3 * m.sin(alpha) - h_s2),
                     (-b / 2 + b_tf / 2 + (s_3 + s_m + s_ef_n) * m.cos(alpha) + s_s2 * m.cos(omega), h - (s_3 + s_m + s_ef_n) * m.sin(alpha) - h_s2)],
                    [(-b_wf / 2 - s_ef_1 * m.cos(alpha), s_ef_1 * m.sin(alpha)),
                     (-b_wf / 2, 0),
                     (-b_wf / 2 + b_ps_ef_e, 0)]]
                reduced_segments = [
                    [(-b_wf / 2 - (s_1 + s_ef_3) * m.cos(alpha) - s_s1 * m.cos(omega), h_1 + h_s1 + s_ef_3 * m.sin(alpha)),
                     (-b_wf / 2 - s_1 * m.cos(alpha) - s_s1 * m.cos(omega), h_3 + h_s1),
                     (-b_wf / 2 - s_1 * m.cos(alpha), h_3),
                     (-b_wf / 2 - s_1 * m.cos(alpha) + s_ef_2 * m.cos(alpha), h_3 - s_ef_2 * m.sin(alpha))],
                    [(-b_wfs_s / 2 - b_wfs / 2 - b_ps_ef_e, 0),
                     (-b_wfs_s / 2 - b_wfs / 2, 0),
            ...
            if n_wfs == 1 and n_ws == 0 and n_tfs == 0:
                main_points = [
                    [(-b / 2, h_wfs),
                     (-b / 2 + b_wfs / 2, 0),
                     (-b / 2 + b_wf / 2, 0),
                     (-b / 2 + b_wf / 2 + (s_m + s_ef_n) * m.cos(alpha), (s_m + s_ef_n) * m.sin(alpha))],
                    [(-b_tf / 2 - s_ef_1 * m.cos(alpha), h - s_ef_1 * m.sin(alpha)),
                     (-b_tf / 2, h),
                     (-b_tf / 2 + b_ps_ef_e,h)]]
                reduced_segments = []
                inactive_segments = [
                ...
                main_points = [
                    ...
                    (-b_tf / 2 + b_ps_ef_e, h)]]
                reduced_segments = [
                    [(-b_tf / 2 - (s_3 + s_ef_3) * m.cos(alpha) - s_s1 * m.cos(omega), h - h_3 - h_s1 - s_ef_3 * m.sin(alpha)),
                    (-b_tf / 2 - s_3 * m.cos(alpha) - s_s1 * m.cos(omega), h - h_3 - h_s1),
                    (-b_tf / 2 - s_3 * m.cos(alpha), h - h_3),
                    (-b_tf / 2 - s_3 * m.cos(alpha) + s_ef_2 * m.cos(alpha), h - h_3 + s_ef_2 * m.sin(alpha))]]
                inactive_segments = [
                    [(-b/2 + b_wf / 2 + (s_m + s_ef_n) * m.cos(alpha), (s_m + s_ef_n) * m.sin(alpha)),
                    (-b_tf / 2 - (s_3 + s_ef_3) * m.cos(alpha) - s_s1 * m.cos(omega), h - h_3 - h_s1 - s_ef_3 * m.sin(alpha))],
                    [(-b_tf / 2 - s_3 * m.cos(alpha) + s_ef_2 * m.cos(alpha), h - h_3 + s_ef_2 * m.sin(alpha)),
                ...
                main_points = [
                    ...
                    (-b / 2 + b_wf_pm / 2 + b_wfs, 0),
                    (-b / 2 + b_wf / 2, 0),
                    (-b / 2 + b_wf / 2 + s_1 * m.cos(alpha), s_1 * m.sin(alpha)),
                    (-b / 2 + b_wf / 2 + s_1 * m.cos(alpha) + s_s2 * m.cos(omega), s_1 * m.sin(alpha) + h_s2),
                    (-b / 2 + b_wf / 2 + (s_1 + s_m + s_ef_n) * m.cos(alpha) + s_s2 * m.cos(omega), (s_1 + s_m + s_ef_n) * m.sin(alpha) + h_s2)],
                    [(-b_tf / 2 - s_ef_1 * m.cos(alpha), h - s_ef_1 * m.sin(alpha)),
                    (-b_tf / 2, h),
                    (-b_tf / 2 + b_ps_ef_e, h)]]
                reduced_segments = [
                ...
                main_points = [
                    ...
                    (-b/2 + b_wfs/2 + b_wf_ps + s_1 * m.cos(alpha), h_1),
                    (-b/2 + b_wfs/2 + b_wf_ps + s_1 * m.cos(alpha) + s_s1 * m.cos(omega), h_1 + h_s1),
                    (-b/2 + b_wfs/2 + b_wf_ps + (s_1 + s_m + s_ef_n) * m.cos(alpha) + s_s1 * m.cos(omega), h_1 + h_s1 + (s_m + s_ef_n) * m.sin(alpha))],
                    [(-b_tf/2 - s_ef_1 * m.cos(alpha), h - s_ef_1 * m.sin(alpha)),
                    (-b_tf/2, h),
                    (-b_tf/2 + b_ps_ef_e, h)]]
                reduced_segments = [
                    [(-b_tf/2 - (s_3 + s_ef_3) * m.cos(alpha) - s_s2 * m.cos(omega), h - h_3 - h_s2 - s_ef_3 * m.sin(alpha)),
                    (-b_tf/2 - s_3 * m.cos(alpha) - s_s2 * m.cos(omega), h - h_3 - h_s2),
                    (-b_tf/2 - s_3 * m.cos(alpha), h - h_3),
                    (-b_tf/2 - s_3 * m.cos(alpha) + s_ef_2 * m.cos(alpha), h - h_3 + s_ef_2 * m.sin(alpha))]]
                inactive_segments = [
                    [(-b / 2 + b_wf / 2 + (s_1 + s_m + s_ef_n) * m.cos(alpha) + s_s2 * m.cos(omega), (s_1 + s_m + s_ef_n) * m.sin(alpha) + h_s2),
    ...
    for i, segment in enumerate(reduced_segments):
        ...
        fig.add_trace(go.Scatter(x=x, y=y,
            mode='lines', line=dict(color=reduced_color, width=4 * chi_d),
...
if orient == 'Вверх':
    sigma_gr = M_sls / W_gr_wf
    sigma_f = M_sls / W_wf_span
else:
    sigma_gr = M_sls / W_gr_tf
    sigma_f = M_sls / W_tf_span

I_f = I_gr - sigma_gr / sigma_f * (I_gr - I_span) # Момент инерции для определения прогибов
...
def draw_beam():
    ...
    for point in points:
        x = [p[0] * UF_Dimensions for p in point]
        y = [p[1] * UF_Dimensions for p in point]
...
def draw_capasity_contour():
    ...
    for point in points:
        ...
        fig.add_trace(go.Scatter(x=x, y=y,
            ...
            fill='tozerox', fillcolor='rgba(0,191,255,0.2)', name='СП 260.1325800.2023'))
    ...
    if number_spans == 1:
        ...
        fig.add_trace(go.Scatter(x=[Q_uls * UF_Forces], y=[M_uls * UF_Moments],
            ...
            showlegend=False, name = f'ULS={U:.2f}')) 
