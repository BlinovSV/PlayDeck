...
if (b_wf / t_nom > 300) or (b_tf / t_nom > 300):
    ...
    st.warnings('Не выполняется требование таблицы 7.1 СП 260.1325800.2023 по соотношению высоты профиля и толщины стенки', icon='⚠️')
elif (0.25 > α / m.pi) or (α / m.pi > 0.5):
    st.warnings('Не выполняется требование таблицы 7.1 СП 260.1325800.2023 по углу наклона стенки', icon='⚠️')
elif max(r, r_s) > 0.04 * t_nom * E_s / R_y:
    st.warnings('Не выполняется требование п.7.2.8 СП 260.1325800.2023', icon='⚠️')
elif (b_wf/t_nom > 250 * h / b_wf) and (b_tf / t_nom > 250 * h / b_tf):
    st.warnings('Не выполняется требование п.7.3.1.2 СП 260.1325800.2023', icon='⚠️')
elif (h / t_nom > 220) or (min(r, r_s) / t_nom > 10):
    st.warnings('Не выполняется требование к профлию для определения несущей способность по поперечной силе', icon='⚠️')
